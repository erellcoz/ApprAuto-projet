...
sulphur_and_phosphorus_columns = [
    "Sulphur_concentration", "Phosphorus_concentration"]
...
def sum_values_inferior_to_value(data):
    ...
    values = data.unique()
    for value in values:
        mean_values[value] = data[data <= value].sum() / \
            data[data <= value].shape[0]
...
def replace_less_than_values(*, df, column, strategy: Literal['max', 'mean']):
    ...
    if df[column].any() and type(df[column].dropna().iloc[0]) == str:
        ...
        df.loc[mask, column] = df.loc[mask,
                                      column].replace('<', '', regex=True)

        if strategy == 'max':
            # Replace the values in the column by the max value
            df.loc[df[new_column] == True, column] = df.loc[df[new_column]
                                                            == True, column].apply(lambda x: float(x))
            ...
            mean_values = sum_values_inferior_to_value(
                df.loc[df[column].notna(), column].apply(lambda x: float(x)))
            df.loc[df[new_column] == True, column].map(mean_values)

        df = df.drop(column + '_<', axis=1)
    return df
...
def split_res_values(value, pattern):
    if isinstance(value, str) and pattern in value:
        ...
        if len(numbers) > 1:
            ...
            # If only one number is before "res"
...
def process_string_values(*, inputs, outputs, strategy, labels_chosen):
    for column in inputs.columns:
        inputs = replace_less_than_values(
            df=inputs, column=column, strategy=strategy)
    inputs['Nitrogen_concentration'], inputs['Nitrogen_concentration_residual'] = zip(
        *inputs['Nitrogen_concentration'].apply(lambda x: split_res_values(x, 'res')))
    if 'Hardness' in labels_chosen:
        outputs['Hardness'], outputs['Hardness_load'] = zip(
            *outputs['Hardness'].apply(lambda x: split_res_values(x, 'Hv')))
    inputs['Interpass_temperature'] = inputs['Interpass_temperature'].apply(
        process_interpass_temperature)
...
def missing_values_sulphur_and_phosphorus(df_train, df_test):
    ...
    df_train.loc[:, sulphur_and_phosphorus_columns] = imp_mean.fit_transform(
        df_train.loc[:, sulphur_and_phosphorus_columns])

    df_test.loc[:, sulphur_and_phosphorus_columns] = imp_mean.transform(
        df_test.loc[:, sulphur_and_phosphorus_columns])
...
def missing_values_other_concentration(df_train, df_test):
    ...
    df_train[other_concentration_columns] = imputer_zero.fit_transform(
        df_train[other_concentration_columns])
    df_test[other_concentration_columns] = imputer_zero.transform(
        df_test[other_concentration_columns])
...
def one_hot_encoding(training_set, testing_set):
    ...
    for column in final_columns:
        if '_nan' in column:
            dummy_columns = [
                col for col in final_columns if col.startswith(column[:-4])]
            for dummy_col in dummy_columns:
                training_encoded.loc[training_encoded[column]
                                     == 1, dummy_col] = np.nan
    ...
    testing_encoded = pd.get_dummies(
        testing_set, drop_first=False, dummy_na=True, dtype=float)
    ...
    testing_encoded = testing_encoded.reindex(
...
def missing_values_physical_properties(train_data, test_data, ordinal_strategy, categorical_strategy, categorical_columns):
    ...
    if len(ordinal_columns) > 0:
        if ordinal_strategy == 'mean':
            ...
            train_data[ordinal_columns], test_data[ordinal_columns] = univariate_imputation(
                train_data[ordinal_columns], test_data[ordinal_columns], 'mean')
        elif ordinal_strategy == 'linear':
            # Replace missing values with a linear regression
            train_data[ordinal_columns], test_data[ordinal_columns] = multivariate_imputation_ordinal(
                train_data[ordinal_columns], test_data[ordinal_columns])

    # Impute categorical values
    if len(categorical_columns) > 0:
        if categorical_strategy == 'most_frequent':
            # Replace missing values with the most frequent value
            train_data[categorical_columns], test_data[categorical_columns] = univariate_imputation(
                train_data[categorical_columns], test_data[categorical_columns], 'most_frequent')
        elif categorical_strategy == 'logistic':
            # Replace missing values with a logistic regression
            train_data[categorical_columns], test_data[categorical_columns] = multivariate_imputation_categorical(
                train_data[categorical_columns], test_data[categorical_columns])
...
def scaler(train_df: pd.DataFrame, test_df: pd.DataFrame, columns_to_normalise: list, strategy: Literal['standard', 'normalizer']):
    if strategy == 'standard':
        ...
        raise ValueError(
            "Invalid strategy. Choose either 'standard' or 'normalizer'.")
    ...
    train_scaled_df = pd.DataFrame(
        train_scaled, columns=columns_to_normalise, index=train_df.index)
    test_scaled_df = pd.DataFrame(
        test_scaled, columns=columns_to_normalise, index=test_df.index)

    # Combine scaled categorical columns with the original DataFrames (excluding the original categorical columns)
    train_final = pd.concat(
        [train_df.drop(columns=columns_to_normalise), train_scaled_df], axis=1)
    test_final = pd.concat(
        [test_df.drop(columns=columns_to_normalise), test_scaled_df], axis=1)
...
def compute_pca(train_df, test_df, pca_percent_explained_variance):
    ...
    # Number of components chosen to explain the specified variance
...
def pipeline_training_set(*, training_set: pd.DataFrame, training_labels: pd.DataFrame, testing_set: pd.DataFrame,
    ...
    training_set, testing_set = missing_values_sulphur_and_phosphorus(
        training_set, testing_set)
    training_set, testing_set = missing_values_other_concentration(
        training_set, testing_set)

    # One Hot Encoding
    training_set, testing_set, categorical_columns = one_hot_encoding(
        training_set, testing_set)

    # Missing values
    training_set, testing_set = missing_values_physical_properties(
        training_set, testing_set, ordinal_strategy=ordinal_strategy, categorical_strategy=categorical_strategy, categorical_columns=categorical_columns)

    # Handle the incompatibility concerning the imputation of some categorical values
    training_set = handle_incompatibility_categorical_features(
    ...
    columns_to_normalise = physical_ordinal_properties_columns + \
        sulphur_and_phosphorus_columns + other_concentration_columns
    training_set_normalised, testing_set_normalised = scaler(
        training_set, testing_set, columns_to_normalise, strategy=scaler_strategy)
    ...
    if pca_columns == 'concentration':
        ...
        pca_columns_list = sulphur_and_phosphorus_columns + \
            other_concentration_columns + physical_ordinal_properties_columns
